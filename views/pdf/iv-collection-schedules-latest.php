...
$lcids = explode(',', $customers[$customer->customer_id]['lcids']);
$last_lcid = array_pop($lcids);
$isCustomerLastContract = $model->lease_contract_id == $last_lcid;
if ($searchModel->hide_collection || $detail->monthly_charge == 0) : ?>
<tr data-cdid="<?= $detail->contract_detail_id ?>" class="type-<?= $model->contract_type . ($isCustomerLastContract ? ' customer-last-contract' : '') ?>">
    <td class="sticky-cell1 border-bottom"><?= $customer->customer_code ?></td>
    <td class="sticky-cell2 border-bottom"><?= $customer->clientContract->repaymentPattern->name ?></td>
    <td class="sticky-cell3 border-bottom"<?= $customerBgColor ? " style=\"background-color:{$customerBgColor}!important;\"" : '' ?>><?= $customer->getName() ?></td>
    <td class="sticky-cell4 border-bottom"><?= $contract->contractNumber ?></td>
    <td class="sticky-cell5 border-bottom"><?= $detail->taxApplication->application_name ?></td>
    <td class="sticky-cell6 border-bottom"><?= $detail->contract_type == 'delinquency' ? '' : $detail->term_start_at ?><br /><?= $detail->term_end_at ?></td>
    <td class="sticky-cell7 border-bottom"><?= $contract->leaseTarget->registration_number ?></td>
    <td class="sticky-cell8 border-bottom"><?= $detail->contract_type == 'delinquency' ? '' : $detail->term_months_count . 'ヶ月' ?><br /><?= $detail->leaseServicer->shorten_name ?></td>

    <td class="sticky-cell9 payment-cell border-bottom">支払</td>
    ...
    <td class="border-bottom"><?= $model->leaseContract->memo ?></td>
</tr>
<?php /* elseif ($searchModel->hide_payment || $detail->monthly_payment == 0) : ?>
    ...
    <tr class="<?= $isCustomerLastContract ? 'customer-last-contract' : '' ?>">
        <td class="sticky-cell6 border-bottom"><?= $detail->term_end_at ?></td>
        <td class="sticky-cell8 border-bottom"></td>
        <td class="sticky-cell9 border-bottom">実績</td>
        <?php foreach($terms as $term) {
            $instance = \app\models\CollectionCell::getInstance($detail->contract_detail_id, $term->term_id);
        ...
        <td class="border-bottom text-end"><?= !$detail->leaseContract->isStoppedOrCancelled ? $detail->getErapsedMonths($lastTerm) : '' $lastMonthlyCharge ? ($lastMonthlyCharge->isLast ? '終' : $lastMonthlyCharge->orderCount + 1) : '終'  ?></td>
...
<?php */ else : ?>
    ...
    <tr class="<?= $isCustomerLastContract ? 'customer-last-contract' : '' ?>">
        <td class="sticky-cell6 border-bottom"><?= $detail->contract_type == 'delinquency' ? '' : $detail->term_end_at ?></td>
        <td class="sticky-cell8 border-bottom"><?= $detail->leaseServicer->shorten_name ?></td>
        <td class="sticky-cell9 payment-cell border-bottom">支払</td>
        <?php foreach($terms as $term) {
            $instance = \app\models\CollectionCell::getInstance($detail->contract_detail_id, $term->term_id);
            if (!$detail->registration_status) {
