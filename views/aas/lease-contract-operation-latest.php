...
<section id="customers">
        ...
        <div class="card">
            ...
            <?php \yii\widgets\Pjax::begin([
                ...
                <?php
                $term = empty($searchModel->target_term) ? (new \DateTime())->setDate((int)($searchModel->target_term_year ?? date('Y')), (int)date('n'), 1)->format('Y年m月') : $searchModel->target_term;
...
EOL;
                ...
                foreach($terms as $the_term) {
                    ...
                    if ($the_term['relative_month'] < -1 || \app\models\TargetTermMonthlyChargeStored::isMonthClosed($the_term['the_term']->format('Y-m-d'), $searchModel->client_corporation_id) ) {
                        $layout .= <<<EOL
                <th>回数</th>
...
EOL;
        ...
        <tbody>
            <tr>
                <td class="sticky-cell1"></td>
                <td class="sticky-cell2"></td>
                <td class="sticky-cell3"></td>
                <td class="sticky-cell4">回収予定合計</td>
                <td colspan="5"></td>
EOL;
                foreach($terms as $the_term) {
                    $total = number_format(\app\models\ContractDetail::getTermTotalChargeAmountWithTax($searchModel, $dataProvider, $the_term), 0);
                    $layout .= <<<EOL
                <td></td>
                <td class="text-end">{$total}</td>
EOL;
                }
                $total = number_format(\app\models\ContractDetail::getTermsDetailsTotalChargeAmountWithTax($searchModel, $dataProvider, $terms), 0);
            ...
            <tr>
                ...
                <td class="sticky-cell2"></td>
                <td class="sticky-cell3"></td>
                <td class="sticky-cell4">回収実績合計</td>
                <td colspan="5"></td>
EOL;
                foreach($terms as $the_term) {
                    $total = number_format(\app\models\ContractDetail::getTermRepaymentTotal($searchModel, $dataProvider, $the_term), 0);
                    $layout .= <<<EOL
                    <td></td>
                    <td class="text-end">{$total}</td>
EOL;
                }
                $total = number_format(\app\models\ContractDetail::getTermsDetailsTotalRepaymentAmountWithTax($searchModel, $dataProvider, $terms), 0);
...
EOL;
                ...
                $total = number_format(\app\models\ContractDetail::getTermsDetailsTotalPaymentAmountWithTax($searchModel, $dataProvider, $terms), 0);
