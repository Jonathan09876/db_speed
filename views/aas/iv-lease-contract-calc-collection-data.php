...
foreach($model->contractDetails as $detail) :
    ...
    <tr>
        ...
        <td class="text-end border-bottom" style="background-color:<?= $bg_color ?>"><?= $lastMonthlyCharges ? number_format(array_sum(array_map(function($monthlyCharge){return $monthlyCharge->temporaryAmountWithTax;}, $lastMonthlyCharges)),0) : '&nbsp;' ?></td>

        <?php $monthlyCharges = (clone $mcQuery)
        ...
        <td class="text-end border-bottom updatable element-cell cell-dropdown"><?= $monthlyCharges ? join('<br />', array_map(function($monthlyCharge){return Html::dropDownList("MonthlyCharge[{$monthlyCharge->monthly_charge_id}][repayment_type_id]", $monthlyCharge->repayment_type_id, \app\models\RepaymentType::getTypes(), ['class' => 'form-control form-select', 'data-id' => $monthlyCharge->monthly_charge_id]);}, $monthlyCharges)) : '&nbsp;' ?></td>
        <td class="text-end border-bottom updatable element-cell cell-text-input"><?= $monthlyCharges ? join('<br />', array_map(function($monthlyCharge){return Html::textInput("MonthlyCharge[{$monthlyCharge->monthly_charge_id}][amount_with_tax]", $monthlyCharge->temporaryAmountWithTax, ['class' => 'form-control formatted', 'data-id' => $monthlyCharge->monthly_charge_id, 'style' => $monthlyCharge->repaymentType->style ?? '']);}, $monthlyCharges)) : '&nbsp;' ?></td>
        <?php else : ?>
        ...
        <td class="text-end border-bottom cell-monthly_charge-temporary_charge_amount"><?= $monthlyCharges ? number_format(array_sum(array_map(function($monthlyCharge){return $monthlyCharge->temporaryAmountWithTax;}, $monthlyCharges)),0) : '&nbsp;' ?></td>
        <?php endif; ?>
