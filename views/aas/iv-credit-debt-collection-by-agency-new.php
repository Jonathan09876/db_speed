...
foreach($model->contractDetails as $detail) :
    ...
    <tr>
        ...
        <?php $monthlyCharge = \app\models\MonthlyCharge::getSibling($detail->contract_detail_id, $targetTerm->format('Ym'), $isNext ? -1 : 0); ?>
    ...
    <tr class="total-row">
        <td colspan="9" class="border-bottom text-center" style="background-color:#ffffdd"><?= $servicer->name ?></td>
        <td class="border-bottom"></td>
        <td class="border-bottom"></td>
        <?php $debt_total_8 = 0; foreach(explode(',', $totals[$servicer->lease_servicer_id]['cdids']) as $cdid) {
            $debt_total_8 += Debt::getTotalDebtBySpan($cdid, $span, 8.0);
        } ?>
        <td class="text-end border-bottom"><?= number_format($debt_total_8, 0) ?></td>
        <?php $debt_total_10 = 0; foreach(explode(',', $totals[$servicer->lease_servicer_id]['cdids']) as $cdid) {
            $debt_total_10 += Debt::getTotalDebtBySpan($cdid, $span, 10.0);
        } ?>
        <td class="text-end border-bottom"><?= number_format($debt_total_10, 0) ?></td>
        <?php $advance_total_8 = 0; foreach(explode(',', $totals[$servicer->lease_servicer_id]['cdids']) as $cdid) {
        ...
        <?php $advance_total_10 = 0; foreach(explode(',', $totals[$servicer->lease_servicer_id]['cdids']) as $cdid) {
        ...
        <td class="border-bottom"></td>
        <td class="border-bottom"></td>
        <?php $client_debt_total_8 = 0; foreach(explode(',', $totals[$servicer->lease_servicer_id]['cdids']) as $cdid) {
        ...
        <?php $client_debt_total_10 = 0; foreach(explode(',', $totals[$servicer->lease_servicer_id]['cdids']) as $cdid) {
