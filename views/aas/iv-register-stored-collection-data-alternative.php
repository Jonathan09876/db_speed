...
$pageSize = $session['page-size-limit'] ?? \app\widgets\PageSizeLimitChanger::DEFAULT_PAGE_SIZE;
if ($index == 0) {
...
$cdids = explode(',',$totals[$customer->customer_id]['cdids']); $fcdid = $cdids[0];
if ($fcdid == $detail->contract_detail_id && $model->isFirstIntermCharges) {
...
?>
    ...
    <tr class="bg-gray">
    <?php else : ?>
    <tr id="mc-<?= $model->monthly_charge_id ?>" data-id="<?= $model->monthly_charge_id ?>" data-index="<?= $index ?>">
    <?php endif; ?>
        ...
        <td class="sticky-cell sticky-cell3 border-bottom"<?= $customerBgColor ? " style=\"background-color:{$customerBgColor}!important;\"" : '' ?>><?= Html::a($customer->name, ['/aas/customer', 'id' => $customer->customer_id]) ?></td>
        <td class="sticky-cell sticky-cell4 border-bottom"><?= Html::a($contract->contractNumber, ['/aas/lease-contract', 'id' => $contract->lease_contract_id]) ?></td>
    ...
    <?php if ($fcdid == $detail->contract_detail_id && $model->isFirstIntermCharges) : ?>
        <td style="vertical-align: middle" class="text-center border-bottom" rowspan="<?= $rowSpan ?>" class="border-bottom">
    ...
    <?php if ($registered) : ?>
        <td class="text-center border-bottom element-cell cell-checkbox"><?php if ($repayment = $model->repayments[0] ?? null) : ?>
            <?php if ($selectedModel->hasRegisteredRepayment($model)) : ?>
                <label>確定済</label>
            <?php else: ?>
            <?= Html::hiddenInput("CancelRepayment[{$repayment->repayment_id}][cancelled]", 0) ?>
            <div><?= Html::checkbox("CancelRepayment[{$repayment->repayment_id}][cancelled]", false, ['value' => 1, 'data-cid' => $customer->customer_id]) ?></div>
            <?php endif; ?>
        <?php endif; ?></td>
    ...
    <?php if ($fcdid == $detail->contract_detail_id && $model->isFirstIntermCharges) : ?>
        <td style="vertical-align: middle" class="text-center border-bottom" rowspan="<?= $rowSpan ?>" class="text-end border-bottom">
