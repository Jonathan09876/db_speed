...
<?php elseif ($searchModel->hide_payment || $detail->monthly_payment == 0) : ?>
    <tr>
        ...
        <?php if (count($monthlyCharges) > 1) :
            ...
            <td class="text-end editable formatted cell-monthly_charge-temporary_charge_amount" data-id="<?= $model->monthly_charge_id ?>"><?= number_format(array_sum(array_map(function($mc){return $mc->amountWithTax;}, $monthlyCharges)),0) ?></td>
        <?php else : ?>
            <td class="text-end" style="background-color:<?= $bgColor ?>"><?= $model->orderCount ?></td>
            <td class="text-end" style="background-color:<?= $bgColor ?>;<?= $model->repaymentType->style ? $model->repaymentType->style : '' ?>"><?= number_format($model->amountWithTax,0) ?></td>
            <?php
        ...
        <?php else : ?>
            ...
            <td class="text-end" style="background-color:<?= $bgColor ?>;<?= $monthlyCharge && $monthlyCharge->repaymentType->style ? $monthlyCharge->repaymentType->style : '' ?>"><?= $monthlyCharge ? number_format($monthlyCharge->amountWithTax,0) : '&nbsp;' ?></td>
            <?php endif; ?>
        <?php endif; endforeach; ?>
    ...
    <tr>
        ...
        <td class="border-bottom">&nbsp;</td>
        <td class="border-bottom text-end"><?= number_format($model->contractDetail->totalChargeAmountWithTax - \app\models\Repayment::getCurrentTotal($detail->contract_detail_id)) ?></td>
        <td class="border-bottom">&nbsp;</td>
...
<?php else : ?>
    <tr>
        ...
        <?php else : ?>
            ...
            <?php endif; ?>
        <?php endif; endforeach; ?>
        <td class="text-end"><?= number_format($model->contractDetail->totalChargeAmountWithTax, 0) ?></td>
        <?php $lease_start_at = (new \DateTime(preg_replace('/(\d+)年(\d+)月/', '$1-$2-1', $model->contractDetail->lease_start_at))); ?>
    ...
    <tr>
        ...
        <td class="text-end"><?= number_format($model->contractDetail->totalChargeAmountWithTax - \app\models\Repayment::getCurrentTotal($detail->contract_detail_id)) ?></td>
    </tr>
    <tr>
        <td class="border-bottom"><?= $detail->term_end_at ?></td>
        ...
        <td class="border-bottom text-end"><?= number_format($model->contractDetail->totalPaymentAmountWithTax, 0) ?></td>
