...
foreach($model->contractDetails as $detail) :
    ...
    <?php if ($detail->contract_detail_id == $totals[$servicer->lease_servicer_id]['last_cdid']) : ?>
    <tr class="total-row">
        <td colspan="9" class="border-bottom text-center" style="background-color:#ffffdd"><?= $customer->name ?></td>
        <td class="border-bottom"></td>
        <td class="border-bottom"></td>
        <td class="text-end border-bottom"><?= number_format($totals[$servicer->lease_servicer_id]['debt_total'], 0) ?></td>
        <td class="text-end border-bottom"><?= number_format($totals[$servicer->lease_servicer_id]['advance_total'], 0) ?></td>
        <td class="border-bottom"></td>
        <td class="border-bottom"></td>
        <td class="border-bottom"></td>
        <td class="text-end border-bottom"><?= number_format($totals[$servicer->lease_servicer_id]['client_debt_total'], 0) ?></td>
    </tr>
    <?php endif ?>
<?php endforeach; ?>