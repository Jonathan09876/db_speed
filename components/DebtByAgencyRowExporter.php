...
class DebtByAgencyRowExporter extends \yii\base\Component
{
    public $model;
    public $searchModel;
    public $targetTerm;
    ...
    public function export()
    {
        ...
        foreach($model->contractDetails as $detail) {
            ...
            $spanTo = \app\models\Term::findOne(['term' => $span['to']->format('Y-m-01')]);
            ...
            $rows[] = $row;
            $last_cdid = explode(',', $this->totals[$servicer->lease_servicer_id]['cdids'])[0];
            if ($detail->contract_detail_id == $last_cdid) {
                ...
                foreach(explode(',', $this->totals[$servicer->lease_servicer_id]['cdids']) as $cdid) {
