...
class MonthlyChargeSearch extends MonthlyCharge
{
    public $target_term;
    public $target_word;
    public $term_from;
    ...
    public function search($params)
    {
        ...
        if (!!$this->calc_collection_data || !!$this->export_collection_data) {
        ...
        else if (!!$this->credit_debt_collection_by_agency || !!$this->credit_debt_collection_by_customer){
            $query = LeaseContract::find()->alias('lc')->distinct()
                ->innerJoin('contract_detail cd', 'cd.lease_contract_id=lc.lease_contract_id')
                ->innerJoin('customer c', 'lc.customer_id=c.customer_id')
            ...
            $query
                ->andWhere(['not',
                    ['or',
                        ['<', 'cd.term_end_at', $lastTerm->format('Y-m-01')],
                        ['>', 'cd.term_start_at', $targetTerm->format('Y-m-t')]
                    ],
                ])
                ->andFilterWhere(['cc.client_corporation_id' => $this->client_corporation_id])
                ->andFilterWhere(['lc.contract_pattern_id' => $this->contract_pattern_id])
