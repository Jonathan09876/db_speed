...
class ImportForm extends \yii\base\Model
{
    ...
    public $file_customer;
    public $file_contract;
    public $file_contract_detail;
    ...
    {
        return [
            [['file_customer', 'file_contract', 'file_contract_detail', 'file_repayment', 'file_payment'], 'file']
        ];
    }
    ...
    {
        return [
            'file_customer' => '顧客情報CSV',
            'file_contract' => '契約情報CSV',
            'file_contract_detail' => '契約詳細情報CSV',
    ...
    {
        $this->file_customer = UploadedFile::getInstance($this, 'file_customer');
        $this->file_contract = UploadedFile::getInstance($this, 'file_contract');
        $this->file_contract_detail = UploadedFile::getInstance($this, 'file_contract_detail');
    ...
    public function loadCustomerFile()
    {
        $path = $this->file_customer->tempName;
        $rows = $this->readCsv($path);
        $model = new ImportCustomer();
        $count = 0;
        while($row = array_shift($rows)) {
            ...
            $instance = new ImportCustomer($attributes);
            if (!$instance->save()) {
    ...
    public function loadContractFile()
    {
        $path = $this->file_contract->tempName;
        $rows = $this->readCsv($path);
    ...
    public function loadContractDetailFile()
    {
        $path = $this->file_contract_detail->tempName;
        $rows = $this->readCsv($path);
    ...
    public function loadPaymentFile()
    {
        if ($this->file_payment) {
            ...
            while($row = array_shift($rows)) {
                ...
                $attributes = array_combine($model->attributes(), $row);
                $instance = new ImportPayment($attributes);
                if (!$instance->save()) {
                    throw new Exception(print_r($instance->firstErrors,1));
                }
            }
    ...
    public function processImportingRecent()
    {
        ...
        try {
            ...
            foreach($query->each() as $row) {
                ...
                foreach($query->each() as $contractRow) {
                    ...
                    echo "+";
                    $contractPattern = ContractPattern::findOne(['code' => $contractRow->contract_pattern, 'removed' => null]);
                    ...
                    foreach($query->each() as $detailRow) {
                        ...
                        $contractDetail = new ContractDetail([
                            ...
                            'collection_start_at' => (new \DateTime($detailRow->term_start_at))->format('Y年n月'),
                            'payment_start_at' => (new \DateTime($detailRow->term_start_at))->format('Y年n月'),
